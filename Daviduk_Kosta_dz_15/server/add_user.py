...
class RegisterUser(QDialog):
    ...
    def __init__(self, database, server):
        ...
        self.label_username = QLabel('Введите имя:', self)
        ...
        self.label_conf = QLabel('Подтверждение:', self)
    ...
    def save_data(self):
        ...
        if not self.client_name.text():
            ...
            return
        elif self.database.check_client(self.client_name.text()):
            ...
            # нижнем регистре.
            password = self.client_passwd.text().encode('utf-8')
            salt = self.client_name.text().lower().encode('utf-8')
            passwd_obj = hashlib.pbkdf2_hmac('sha512', password, salt, 100000)
            # print(type(passwd_obj))
            print(binascii.hexlify(passwd_obj))
            self.database.add_user(self.client_name.text(), binascii.hexlify(passwd_obj))
            LOG.debug(
                f'"Регистрация - Успех" - имя: "{self.client_name.text()}"')
            LOG.debug(
                f'"Хэш пароля: "{binascii.hexlify(passwd_obj)}"')
