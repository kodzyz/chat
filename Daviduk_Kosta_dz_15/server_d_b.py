...
class ServerBase:
    ...
    def __init__(self):
        self.engine = create_engine('sqlite:///server.db3', echo=False, pool_recycle=7200,
    ...
    def save_log(self, login, info, key):
        '''
        Если log есть в ActiveClient заполняем только Story
        ('ip_address', 'client_id' берем из Client)
        если нет - добавляем log в ActiveClient и заполняем Story
        '''
