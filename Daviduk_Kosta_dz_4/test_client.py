import unittest
from client import createParser, pars_listen
import argparse
...
class TestClient(unittest.TestCase):
    ...
    def test_pars_listen_OK(self):
        self.assertEqual(pars_listen({"response": 200}), 'response: 200')

    def test_pars_listen_Bad(self):
        self.assertEqual(pars_listen({"response": "20"}), 'response: 400')
